...
DMOps<DenseMatrix>& MatOps=DMOps<DenseMatrix>::getInstance();
...
TEST(DMOpsTest, MatrixMultiplication) {
    ...
    DenseMatrix mat = MatOps.matMul(matA, matB);
...
TEST(DMOpsTest, ElementMult) {
    ...
    DenseMatrix mat = MatOps.elemMult(matA, matB);
...
TEST(DMOpsTest, ElementAdd) {
    ...
    DenseMatrix mat = MatOps.elemAdd(matA, matB);
...
TEST(DMOpsTest, ElementSub) {
    ...
    DenseMatrix mat = MatOps.elemSub(matA, matB);
...
TEST(DMOpsTest, ElementDiv) {
    ...
    DenseMatrix mat = MatOps.elemDiv(matA, matB);
...
TEST(DMOpsTest, ScalarMultiplication) {
    ...
    DenseMatrix mat = MatOps.scalarMult(matA, 5);
